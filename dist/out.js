...
document.addEventListener('DOMContentLoaded', () => {
        ...
        class Bullet {
            ...
            update() {
                if (ctrlPressed === true) {
                    ...
                    this.y = spaceship.layers[0].y + p * 8;
                    this.x = spaceship.layers[0].x;
                }  
        ...
        let init = () => {
            ...
            // Spaceship
            let xStartingPoint = innerWidth/2 + p;
            let yStartingPoint = innerHeight - 110 + p;
                ...
                if (asteroidsControlPoint === true ) {
                    for (let i = 0; i < asteroidsNumber; i++) {
                        if (asteroidsArray[i] !== undefined) {
                            ...
                            if (getDistance(bullet.x, bullet.y, asteroidXCenter, asteroidYCenter) < 35) {
                                let index = asteroidsArray.indexOf(asteroidsArray[i]);
                                asteroidsArray.splice(index, 1);
                                asteroidsNumber += 0.5;
                            }
                            if (asteroidsArray[i] !== undefined && asteroidsArray[i].y > innerHeight){
                                console.log('earth destroyed');
